...
Status FieldToNode(const std::shared_ptr<Field>& field,
  ...
  switch (field->type()->id()) {
    ...
    case ArrowType::DICTIONARY: {
      ...
      const ::arrow::DictionaryType& dict_type = static_cast<const ::arrow::DictionaryType&>(*field->type());
      std::shared_ptr<::arrow::Field> unpacked_field = ::arrow::field(field->name(), dict_type.dictionary()->type(), field->nullable(), field->metadata());
