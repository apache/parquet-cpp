...
void ParquetFileReader::ParseMetaData() {
  ...
  uint8_t* metadata_buffer = new uint8_t[metadata_len];
  try {
    buffer_->Read(metadata_len, metadata_buffer, &bytes_read);
    if (bytes_read != metadata_len) {
      throw ParquetException("Invalid parquet file. Could not read metadata bytes.");
    }
    DeserializeThriftMsg(metadata_buffer, &metadata_len, &metadata_);
