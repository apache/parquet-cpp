...
ColumnReader* RowGroupReader::Column(size_t i) {
  ...
  ScopedInMemoryInputStream* pInput = static_cast<ScopedInMemoryInputStream*>(input.get());
  size_t bytes_read = source->Read(pInput->size(), pInput->data());
  if (bytes_read != pInput->size()) {
  ...
  std::shared_ptr<ColumnReader> reader = ColumnReader::Make(&col.meta_data,
      &this->parent_->metadata_.schema[i + 1], input);
