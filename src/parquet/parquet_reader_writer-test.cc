 ...
 public:
  ...
  void WriteParquet() {
    ...
    for (size_t i = 0; i < fields_.size(); i++) {
      auto column_writer = static_cast<parquet::TypedColumnWriter<TestType>*>(rg_writer->NextColumn());
  ...
  void VerifyParquetStats() {
    ...
    std::unique_ptr<parquet::ParquetFileReader> parquet_reader =
        parquet::ParquetFileReader::Open(std::make_shared<arrow::io::BufferReader>(pbuffer));

    // Get the File MetaData
    std::shared_ptr<parquet::FileMetaData> file_metadata = parquet_reader->metadata();
    std::shared_ptr<parquet::RowGroupMetaData> rg_metadata = file_metadata->RowGroup(0);
    for (size_t i = 0; i < fields_.size(); i++) {
      std::shared_ptr<parquet::ColumnChunkMetaData> cc_metadata = rg_metadata->ColumnChunk(i);
...
void TestStatistics<Int32Type>::SetValues() {
  ...
  stats_[0].set_min(std::string(reinterpret_cast<const char*>(&values_[5]), sizeof(T)))
    .set_max(std::string(reinterpret_cast<const char*>(&values_[4]), sizeof(T)));

  // Write INT32 values and min,max
  stats_[1].set_min(std::string(reinterpret_cast<const char*>(&values_[0]), sizeof(T)))
...
void TestStatistics<FloatType>::SetValues() {
  for (int i = 0; i < NUM_VALUES; i++) {
    values_[i] = (i * 1.0f) - 5; // {-5.0, -4.0, -3.0, -2.0, -1.0, 0.0, 1.0, 2.0, 3.0, 4.0};
  }

  // Write Float values and min,max
  stats_[0].set_min(std::string(reinterpret_cast<const char*>(&values_[0]), sizeof(T)))
    .set_max(std::string(reinterpret_cast<const char*>(&values_[9]), sizeof(T)));
}
...
void TestStatistics<ByteArrayType>::AddNodes(std::string name) {
    ...
    fields_.push_back(schema::PrimitiveNode::Make(name, Repetition::REQUIRED, Type::BYTE_ARRAY,
                                                 LogicalType::UTF8));
}
...
void TestStatistics<ByteArrayType>::SetValues() {
  ...
  stats_[0].set_min(std::string(reinterpret_cast<const char*>(vals[2].c_str()), max_byte_array_len))
    .set_max(std::string(reinterpret_cast<const char*>(vals[9].c_str()), max_byte_array_len));
}
...
void TestStatistics<FLBAType>::AddNodes(std::string name) {
    ...
    fields_.push_back(schema::PrimitiveNode::Make(name, Repetition::REQUIRED, Type::FIXED_LEN_BYTE_ARRAY,
...
void TestStatistics<FLBAType>::SetValues() {
  ...
  // Write String values and min,max
  stats_[0].set_min(std::string(reinterpret_cast<const char*>(vals[1].c_str()), FLBA_LENGTH))
